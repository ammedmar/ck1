...
In this case, the complex \((W, d_W)\) is isomorphic to the subcomplex \((\img(i \circ p), d_V)\).\footnote{
	Explicitly, \(i\) is injective and \(p\) is surjective since \(p \circ i = \id_W\).
	Obviously, \(i\) is surjective onto its image \(W' = i(W)\) so it suffices to show that \(W' = (i \circ p)(V)\), which follows from the surjectivity of \(p\).
}
